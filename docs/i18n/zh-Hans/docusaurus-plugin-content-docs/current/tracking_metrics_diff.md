...
本文记录 `scripts/tracking_experiment/compute_tracking_metrics.py` 已实现的
正式指标契约，并逐项说明它与当前仓库旧指标、Spider G1 WBC 指标和 SONIC
评测指标的差异。目标是在同一条 motion 或同一个 motion 数据集上，对多个
controller 产生公平、可审计、可复算的 rollout 对比结果。
...
保存轨迹时的 body layout 现在固定来自 G1 robot config，不再取决于具体
controller YAML。最终使用 robot model 与 reference motion 共有的交集。
